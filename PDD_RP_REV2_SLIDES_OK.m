...
% title('Comparação MLT');
% ylabel('Afluência [m³/s]');
% xlabel('Mês');
% ylim([0 2000])
Y_AUX=[];
...
for t=T:-1:1    
    ...
    for i=1:length(X) %i=estado no estágio atual (x)
        for j=1:length(X) %j=estado no estágio seguinte(x_next)   
...
%%
%===SIMULAÇÃO A PARTIR DE X_atual e Y_DATA============
%===========================================================
X_atual=X(end); %partindo de reservatório cheio
% X_atual=X(1); %partindo de reservatório vazio
mes=1;
...
% ylabel(ax_volume(2), 'Afluência [m³/s]');
% set(afluencia,'linestyle','--');
% legend('72 meses','60 meses','Afluência');
% hold on
% plot(x_axis,[ones(1,T)*Xmin;ones(1,T)*Xmax],'Color',[0 0 0],'LineStyle',':')
% hold off
% 
...
disp(strcat('Razão vertimento PDD/Hydrolab=',num2str(sum(Vertimento_otimo(1,:)/sum(Vertimento_otimo(2,:))))))
disp(strcat('Razão custo PDD/Hydrolab=',num2str(Custo_otimo(1,1)/Custo_otimo(2,1))))
disp(strcat('Razão produção hidroeletrica PDD/Hydrolab=',num2str(sum(PROD(1,:))/sum(PROD(2,:)))))
